...
/// Render raw address bytes as a bech32 string, falling back to a hex encoding
/// if the bytes are not a parseable address. Never panics.
pub fn address_bytes_to_string(bytes: &[u8]) -> String {
    Address::from_bytes(bytes)
        .ok()
